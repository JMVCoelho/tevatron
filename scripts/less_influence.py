...
class LESSQueryAttribution():
    def __init__(self, valid_gradient_path, optim_state_path, model_args, training_args, data_args):
        ...
        self.validation_gradient = self.get_validation_gradient(valid_gradient_path).cuda()
        ...
        self.dataset = load_dataset("XBKYS/minicpm-embedding-data", data_files=data_files, split="train", cache_dir=data_args.dataset_cache_dir)
        print(f"Sharding in {data_args.dataset_number_of_shards} shards.")
        print(f"Processing shard {data_args.dataset_shard_index}.")
        self.dataset = self.dataset.shard(num_shards=data_args.dataset_number_of_shards, index=data_args.dataset_shard_index)
    ...
    def get_validation_gradient(self, valid_gradient_path):
        ...
        print(f"Loaded valid gradient. Shape: {grad.shape}. Norm: {torch.norm(grad, p=2).item()}")
        return grad
    ...
    def get_less_score(self, example) -> list[str]:        
        ...
        # random_example = self.dataset[random_index]
        # more_negatives = random_example["neg"][1:]
    ...
    def run(self, outpath):
        ...
        with open(outpath, 'w') as h1:
                for example in tqdm(self.dataset):
                    ...
                    query =  example["query"][1].replace("\t", "").replace("\n", "").replace("\r", "")
...
def main():
    ...
    attribution_method.run(outpath=f"{training_args.output_dir}_{data_args.dataset_shard_index}")
