from transformers import T5Model, T5Config, TrainingArguments, Trainer, AutoTokenizer, AutoModelForSequenceClassification
...
train_data_path = "/data/user_data/jmcoelho/embeddings/marco_docs/pythia-160m-marco-docs-bow-ct-pretrain-bs256-all-queries-10k2-valid-5-group-level-T1/group_level_10000_two_valid_orcale/datamodel_train_binary_easy_hard_separation.tsv"
val_data_path =  "/data/user_data/jmcoelho/embeddings/marco_docs/pythia-160m-marco-docs-bow-ct-pretrain-bs256-all-queries-10k2-valid-5-group-level-T1/group_level_10000_two_valid_orcale/datamodel_val_binary_easy_hard_separation.tsv"
test_data_path = "/data/user_data/jmcoelho/embeddings/marco_docs/pythia-160m-marco-docs-bow-ct-pretrain-bs256-all-queries-10k2-valid-5-group-level-T1/group_level_10000_two_valid_orcale/datamodel_test_binary_easy_hard_separation.tsv"
model_out=f"temp"
...
class DatamodelDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        tokenized_sentences = self.tokenizer(
            [QID2TEXT[query] + DID2TEXT[docs[0]]],
        ...
        tokenized_sentences['label'] = torch.tensor(int(label), dtype=torch.long)
...
def compute_metrics(preds):
    ...
    predicted_labels = torch.argmax(torch.tensor(p), dim=1)
    accuracy = (predicted_labels == torch.tensor(l)).float().mean().item()
...
#model = T5FusionRegression(model_to_train, n_fusion=10) # 1pos 9neg 1query
model = AutoModelForSequenceClassification.from_pretrained(model_to_train, num_labels=2) # 1pos 9neg 1query
