...
class DistilCollator:
    ...
    def __call__(self, features: List[Tuple[str, List[str]]]):
        ...
        all_queries = [f[0] for f in features]
        all_passages = []
        #all_scores = torch.tensor([f[2] for f in features])

        teacher_query_embeddings = torch.tensor([f[2] for f in features])
        teacher_passage_embeddings = torch.tensor([f[3] for f in features])
        teacher_passage_embeddings = teacher_passage_embeddings.view(teacher_passage_embeddings.size(0) * teacher_passage_embeddings.size(1), -1)
        ...
        #return q_collated, d_collated, scores
        return q_collated, d_collated, teacher_query_embeddings, teacher_passage_embeddings
