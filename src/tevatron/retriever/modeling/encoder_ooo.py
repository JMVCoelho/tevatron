...
class EncoderModel(nn.Module):
    ...
    def forward(self, query: Dict[str, Tensor] = None, passage: Dict[str, Tensor] = None):
        ...
        p_reps, ld_p_reps = None, None
        if passage:
            p_reps, ld_p_reps = self.encode_passage(passage) 
        ...
        if self.training:
            if self.is_ddp:
                ...
                ld_p_reps = self._dist_gather_tensor(ld_p_reps)
                dist_input_ids = self._dist_gather_tensor(passage['input_ids'])
                dist_attn_mask = self._dist_gather_tensor(passage['attention_mask'])

            bow_loss = None
            full_weights = torch.zeros([p_reps.shape[0], 50304], device=p_reps.device)
            for idx, _input_ids in enumerate(dist_input_ids):
            ...
            projections = self.lm_head(p_reps) #batchsize, vocab_size
            ...
            for idx, _input_ids in enumerate(dist_input_ids):
                quarter_size =  (dist_attn_mask[idx].sum()//N).item()
            ...
            sub_embedding_projections = torch.unbind(self.lm_head(ld_p_reps), dim=1)
    ...
    def build(
        ...
        lm_head = None
        if model_args.matrioshka_reg or model_args.pooling == "landmark":
