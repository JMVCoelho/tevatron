...
PARALLELIZE_DOCSTRING = r"""
    ...
    model = T5ForConditionalGenerationRoPE.from_pretrained("t5-3b")
    device_map = {
        0: [0, 1, 2],
        1: [3, 4, 5, 6, 7, 8, 9],
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            if hasattr(module.wo, "bias") and module.wo.bias is not None:
                module.wo.bias.data.zero_()
        elif isinstance(module, T5AttentionRoPE):
...
class T5ModelRoPE(T5PreTrainedModel):
...
class T5ModelRoPEFusion(T5ModelRoPE):
    def __init__(self, config: T5Config, n_fusion):
        super().__init__(config)
    ...
    def forward(
        ...
        fusion_parameter = self.n_fusion 
        dim = hidden_states.size(1)
        len_decoding = len(decoder_input_ids)
        fused_samples = int(len_decoding) // fusion_parameter
        ...
        decoder_input_ids = decoder_input_ids[list(range(0,len_decoding, fusion_parameter))]
...
class T5ForConditionalGenerationRoPE(T5PreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        r"encoder.embed_tokens.weight",
        r"decoder.embed_tokens.weight",
        r"lm_head.weight",
    ...
    def forward(
        ...
        ```python
        >>> from transformers import T5Tokenizer, T5ForConditionalGenerationRoPE

        >>> tokenizer = T5Tokenizer.from_pretrained("t5-small")
        >>> model = T5ForConditionalGenerationRoPE.from_pretrained("t5-small")
