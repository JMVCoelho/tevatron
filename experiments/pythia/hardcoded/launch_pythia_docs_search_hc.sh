...
set -f && OMP_NUM_THREADS=24 python -m tevatron.retriever.driver.search_gpu \
    --query_reps $EMBEDDING_OUTPUT_DIR/$trained_model_name/query-gen11.pkl \
    --passage_reps $EMBEDDING_OUTPUT_DIR/$trained_model_name/corpus*.pkl \
    --depth 100 \
    --batch_size 128 \
    --save_text \
    --save_ranking_to $EMBEDDING_OUTPUT_DIR/$trained_model_name/run.gen11.txt


python src/tevatron/utils/format/convert_result_to_trec.py \
    --input $EMBEDDING_OUTPUT_DIR/$trained_model_name/run.gen11.txt \
    --output $EMBEDDING_OUTPUT_DIR/$trained_model_name/run.gen11.trec


python scripts/eval_trec.py /data/user_data/jmcoelho/datasets/marco/documents/qrels.gen11.tsv $EMBEDDING_OUTPUT_DIR/$trained_model_name/run.gen11.trec