...
OUTPUT_FILE=$EMBEDDING_OUTPUT_DIR/$trained_model_name/query-gen2-mates-dpo7-$subset.pkl
...
if [ -f "$OUTPUT_FILE" ]; then
  ...
  python -m tevatron.retriever.driver.encode \
    ...
    --dataset_path /data/user_data/jmcoelho/datasets/llama_generator/dpo_mates_8/processed.jsonl \
    --query_max_len 32 \
    --passage_max_len 512 \
    --encode_output_path $OUTPUT_FILE
fi

    #--dataset_path "/data/user_data/jmcoelho/datasets/llama_generator/${subset}_score_subset.jsonl" \
    #--dataset_path "/data/user_data/jmcoelho/datasets/llama_generator/processed.jsonl" \
